...
<div>
    ...
    <p id="debug-info"></p>
    <p class="info">마우스를 움직여 노란색 원(질량체)을 조작하세요. 흰색 별들이 중력렌즈 효과로 링 또는 아크 형태로 왜곡됩니다.</p>
</div>
<script>
try {{
    ...
    const G = 6.67430e-11;
    const c = 3e8;
    const M = {mass_scale} * 1e12 * 1.989e30; // 슬라이더로 조절된 질량
    const D_LS = 1e25; // 렌즈-소스 거리 (임의, 천문 단위)
    ...
    const stars = Array.from({{ length: 100 }}, () => {{
        ...
        const radius = Math.random() * 200 + 100; // 질량체 주변에 분포
        return {{
    ...
    function calculateDeflectionAngle(x, y, lensX, lensY) {{
        ...
        // 아인슈타인 반경: theta_E = sqrt((4GM/c^2) * (D_LS / (D_L * D_S)))
        const theta_E = Math.sqrt((4 * G * M / (c * c)) * (D_LS / (D_L * D_S))) * 1e12; // 픽셀 단위 조정
        // 렌즈 방정식: beta = theta - theta_E^2 / theta
        const theta = r / 100; // 픽셀 단위로 정규화
        const beta = theta - (theta_E * theta_E) / theta;
        const angle = Math.atan2(dy, dx);
        // 왜곡된 위치 (아인슈타인 링 근사)
        const newX = lensX + Math.cos(angle) * beta * 100;
        const newY = lensY + Math.sin(angle) * beta * 100;
        // 캔버스 내로 좌표 제한
    ...
    function draw() {{
        ...
        ctx.beginPath();
        ctx.arc(lensPos.x, lensPos.y, 15, 0, 2 * Math.PI);
        ctx.fill();
        ...
        stars.forEach(star => {{
            ...
            ctx.beginPath();
            ctx.arc(distorted.x, distorted.y, 2, 0, 2 * Math.PI); // 작은 별
            ctx.fill();
        }});
...
st.markdown("""
### 시뮬레이션 안내
마우스를 움직여 노란색 원(질량체)을 조작하고, 슬라이더로 질량을 조절하세요. 흰색 별들이 질량체 주변에서 아인슈타인 링 또는 아크 형태로 왜곡됩니다.
